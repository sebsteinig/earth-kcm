...
(function() {
    ...
    var MIN_MOVE = 1;                         // slack before a drag operation beings (pixels)
...
//    var FRAME_RATE = 40;                      // desired milliseconds per frame
    ...
    // configuration of default experiment
//    var DPD = 1400
//    var FPG = 2
//    var WR  = 1000
//    var CO2 = 1200

    var DPD = (typeof DPD === 'undefined') ? 1400 : DPD;
    var FPG = (typeof FPG === 'undefined') ? 2 : FPG;
    var WR  = (typeof WR === 'undefined') ? 1000 : WR;
    var CO2 = (typeof CO2 === 'undefined') ? 1200 : CO2;
...
//     console.log(CO2);
    ...
    function buildMesh(resource) {
        ...
        return µ.loadJson(resource).then(function(topo) {
            ...
            var o = topo.objects;
            var coastC50 = topojson.feature(topo, o.C50_coastlines);
            var coastC53 = topojson.feature(topo, o.C53_coastlines);
            var coastW07 = topojson.feature(topo, o.W07_coastlines);
//            var coastLo = topojson.feature(topo, o.coastline_50m);
...
//               lakesHi: lakesHi
    ...
    function buildGrids() {
        ...
        var cancel = this.cancel;
        downloadsInProgress++;
        var expid = configuration.get("date")
        switch (expid) {
          case "C54mC50":
          case "C54mC53":
          case "C63mC53":
            var loaded = when.map(products_diff.productsFor(configuration.attributes), function(product) {
            ...
            inputController, {
                moveStart: function() {
                    ...
                    switch (expid) {
                        ...
                        case "WC5":
                        case "WC6":
                        coastline.datum(mesh.coastC53);
                        break;
                        case "W04":
                        coastline.datum(mesh.coastW07);
                        break;
                        default:
                        coastline.datum(mesh.coastC53);
                        break;
  ...
  //                  lakes.datum(mesh.lakesLi);
    ...
    function drawOverlay(field, overlayType) {
        ...
        if (grid) {
            ...
            // Draw permanent color bar for reference.
//             var colorBar_permanent = d3.select("#scale-permanent"), scale = grid.scale, bounds = scale.bounds;
//             var c = colorBar_permanent.node(), g = c.getContext("2d"), n = c.width - 1;
//             for (var i = 0; i <= n; i++) {
//                 var rgb = scale.gradient(µ.spread(i / n, bounds[0], bounds[1]), 1);
//                 g.fillStyle = "rgb(" + rgb[0] + "," + rgb[1] + "," + rgb[2] + ")";
//                 g.fillRect(i, 0, 1, c.height);
...
// 
//             // Show tooltip on hover.
//             colorBar_permanent.on("mousemove", function() {
//                 var x = d3.mouse(this)[0];
//                 var pct = µ.clamp((Math.round(x) - 2) / (n - 2), 0, 1);
//                 var value = µ.spread(pct, bounds[0], bounds[1]);
//                 var elementId = grid.type === "wind" ? "#location-wind-units" : "#location-value-units";
//                 var units = createUnitToggle(elementId, grid).value();
//                 colorBar_permanent.attr("title", µ.formatScalar(value, units) + " " + units.label);
//             });
        }
    }
...
//           description = od.name + od.qualifier;
            description = "KCM simulations";
//            center = grids.overlayGrid.source;
            center = "Kiel Climate Model in Early Cretaceous configuration";
    ...
    function init() {
        ...
        var label_permanent = d3.select("#scale-label-permanent").node();
//         d3.select("#scale-permanent")
//             .attr("width", (d3.select("#menu").node().offsetWidth - label.offsetWidth) * 0.97)
//             .attr("height", label.offsetHeight * 2);

        d3.select("#show-menu").on("click", function() {
            if (µ.isEmbeddedInIFrame()) {
                window.open("https://kcm-earth-app.herokuapp.com/" + window.location.hash, "_blank");
            }
            else {
        ...
        gridAgent.listenTo(configuration, "change", function() {
            ...
            if (_.intersection(changed, ["hour", "param", "surface", "level"]).length > 0) {
...
//                globeAgent.submit(buildGlobe, configuration.get("projection"));
        ...
        gridAgent.listenTo(configuration, "change", function() {
            ...
            // Build a new grid if any layer-related attributes have changed.
            if (_.intersection(changed, ["date"]).length > 0) {
                stopCurrentAnimation(true);  // cleanup particle artifacts over continents
                gridAgent.submit(buildGrids);
                globeAgent.submit(buildGlobe, configuration.get("projection"));
            }
...
//             switch (mode) {
//                 case "wind":
//                     d3.select("#change-to-C51").attr("title", "show C51");
//                     d3.select("#change-to-C53").attr("title", "show C53");
//                     d3.select("#change-to-C54").attr("title", "show C54");
//                     break;
//                 case "ocean":
//                     d3.select("#change-to-C51").attr("title", "show C51");
//                     d3.select("#change-to-C53").attr("title", "show C53");
...
//             }
        ...
        d3.select("#ocean-mode-enable").on("click", function() {
            if (configuration.get("param") !== "ocean") {
                configuration.save({param: "ocean", surface: "depthz", level: "0m", overlayType: "ocevel"});
                stopCurrentAnimation(true);  // cleanup particle artifacts over continents
            }
        });
        configuration.on("change:param", function(x, param) {
        ...
        d3.select("#PD-mode-enable").on("click", function() {
            if (configuration.get("date") !== "W04") {
//                 d3.selectAll(".CRET-mode").classed("invisible");
                configuration.save({period: "PD",date: "W04"});
                stopCurrentAnimation(true);  // cleanup particle artifacts over continents
            }
        });

        d3.select("#CRET-mode-enable").on("click", function() {
            if (configuration.get("date") !== "C53") {
//               d3.selectAll(".PD-mode").classed("invisible");
              configuration.save({period: "CRET",date: "C53"});
              stopCurrentAnimation(true);  // cleanup particle artifacts over continents
            }
        });


        configuration.on("change:date", function(x, date) {
            var expid = configuration.get("date")
            d3.selectAll(".CRET-mode").classed("invisible", expid === "W04");
            d3.select("#PD-mode-enable").classed("highlighted", date === "W04");
            d3.selectAll(".PD-mode").classed("invisible", expid !== "W04");
            d3.select("#CRET-mode-enable").classed("highlighted", date !== "W04");
        });

        // Add event handlers for the Drake Passage buttons.
        d3.select("#change-to-D0").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(0,config_old[1],config_old[2],config_old[3])
          configuration.save({date: expid_new}); 
        });
        d3.select("#change-to-D200").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(200,config_old[1],config_old[2],config_old[3])
          configuration.save({date: expid_new}); 
        });
        d3.select("#change-to-D1400").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(1400,config_old[1],config_old[2],config_old[3])
              configuration.save({date: expid_new}); 
        });

        // Add event handlers for the Falkland Plateau buttons.
        d3.select("#change-to-FPG1").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(config_old[0],1,config_old[2],config_old[3])
          configuration.save({date: expid_new}); 
        });
        d3.select("#change-to-FPG2").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(config_old[0],2,config_old[2],config_old[3])
          configuration.save({date: expid_new}); 
        });
        d3.select("#change-to-FPG3").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(config_old[0],3,config_old[2],config_old[3])
              configuration.save({date: expid_new}); 
        });

        // Add event handlers for the Walvis Ridge buttons.
        d3.select("#change-to-WRS").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(config_old[0],config_old[1],200,config_old[3])
          configuration.save({date: expid_new}); 
        });
        d3.select("#change-to-WRI").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(config_old[0],config_old[1],1000,config_old[3])
          configuration.save({date: expid_new}); 
        });

        // Add event handlers for the CO2 buttons.
        d3.select("#change-to-600ppm").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(config_old[0],config_old[1],config_old[2],600)
          configuration.save({date: expid_new}); 
        });
        d3.select("#change-to-1200ppm").on("click", function() { 
          var expid_old = configuration.get("date")
          var config_old = GetConfigKCM(expid_old)
          var expid_new = GetExpID(config_old[0],config_old[1],config_old[2],1200)
          configuration.save({date: expid_new}); 
        });


        configuration.on("change:date", function(x, date) {
            var configKCM = GetConfigKCM(date)
            d3.select("#change-to-D0").classed("highlighted", configKCM[0] === 0);
            d3.select("#change-to-D200").classed("highlighted", configKCM[0] === 200);
            d3.select("#change-to-D1400").classed("highlighted", configKCM[0] === 1400);
            d3.select("#change-to-FPG1").classed("highlighted", configKCM[1] === 1);
            d3.select("#change-to-FPG2").classed("highlighted", configKCM[1] === 2);
            d3.select("#change-to-FPG3").classed("highlighted", configKCM[1] === 3);
            d3.select("#change-to-WRS").classed("highlighted", configKCM[2] === 200);
            d3.select("#change-to-WRI").classed("highlighted", configKCM[2] === 1000);
            d3.select("#change-to-600ppm").classed("highlighted", configKCM[3] === 600);
            d3.select("#change-to-1200ppm").classed("highlighted", configKCM[3] === 1200);
        });
  ...
  //      bindButtonToConfiguration("#animate-currents", {param: "ocean", surface: "surface", level: "currents"});
        d3.selectAll(".depth").each(function() {
            var id = this.id, parts = id.split("-");
        ...
        bindButtonToConfiguration("#overlay-currents", {overlayType: "ocevel"});
